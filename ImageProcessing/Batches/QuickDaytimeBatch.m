...
%     Gray_Image = imread([params('image_dir'),'/',filename]);
    ...
    %% For Non-Tiff images
    %NOTE THIS FUNCTION CALLS 'params('IMAGE DIR')' SO YOU MUST SET THAT
    %VALUE IN >>>params.m
...
%     imwrite(image,[params('image dir'),'/Gray/TIFFconverted/',strrep(filename,'.dat','.tif')],'tif');
    ...
    if ~exist([params('image dir'),'/Gray/Col_Sub'],'dir')
        mkdir([params('image dir'),'/Gray/Col_Sub']);
    end
    imwrite(DC_image,[params('image_dir'),'/Gray/Col_Sub/',strrep(filename,'.dat','_DC.tif')],'tif');
