...
files = dir([params('image dir'),'/img*.dat']);
...
for ii = 1:length(files)
    ...
    if ~exist([params('Image Dir'),'/TIFFconverted/',strrep(filename,'.dat','.tif')],'file')
            I = LoadDayStarImage(filename);

            
    end
    
               filename = strrep(filename,'.dat','.tif'); 
