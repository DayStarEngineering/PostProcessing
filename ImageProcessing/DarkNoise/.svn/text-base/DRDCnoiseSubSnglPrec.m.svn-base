...
function [colsSubImage, colsAndRowsSubImage] = DRDCnoiseSubSnglPrec(image)
%% Set up
% dark cols and rows
...
DCdataTop_L = image(rowStartTop:rowEndTop,DCsStartLeft:DCsEndLeft); % first 16 cols
DCdataTop_R = image(rowStartTop:rowEndTop,DCsStartRight:DCsEndRight); % last 16 cols
...
DCdataBott_L = image(rowStartBott:rowEndBott,DCsStartLeft:DCsEndLeft); % first 16 cols
DCdataBott_R = image(rowStartBott:rowEndBott,DCsStartRight:DCsEndRight); % last 16 cols
...
DSubSecTop_L = DRdataTop(1:numDRs,1:numDCs);
DSubSecBott_L = DRdataBott(1:numDRs,1:numDCs);
...
for jj = 1:numDRs
    if (LEFT_ONLY)
        DRNoiseTop(jj)  = mean(DSubSecTop_L(jj,:));
        DRNoiseBott(jj) = mean(DSubSecBott_L(jj,:));
    else
        DRNoiseTop(jj)  = mean([ mean(DSubSecTop_L(jj,:)) , mean(DSubSecTop_R(jj,:)) ]);
        DRNoiseBott(jj) = mean([ mean(DSubSecBott_L(jj,:)) , mean(DSubSecBott_R(jj,:)) ]);
    end
end
...
for ii = DCsStartLeft:DCsEndRight
    topDRColAvgs(ii)    = mean(DRdataTop(:,ii));
    bottDRColAvgs(ii)   = mean(DRdataBott(:,ii));
end
